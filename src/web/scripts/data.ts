...
const LIKES_KEY = "streetfood_likes";
const MY_LIKES_KEY = "streetfood_my_likes";
const BOOKMARKS_KEY = "streetfood_bookmarks";
const MY_BOOKMARKS_KEY = "streetfood_my_bookmarks";
const SEEDED_KEY = "streetfood_social_seeded";

const SEED_LIKES: Record<string, number> = {
  "karim-delhi-ghi789": 47,
  "paradise-biryani-hyderabad-yza567": 38,
  "vidyarthi-bhavan-bangalore-stu901": 62,
  "ashok-vada-pav-mumbai-mno345": 29,
  "lassiwala-jaipur-bcd890": 21,
  "peter-cat-kolkata-nop012": 33,
  "kc-das-kolkata-qrs345": 15,
  "kulcha-land-amritsar-klm789": 24,
};

const SEED_BOOKMARKS: Record<string, number> = {
  "karim-delhi-ghi789": 18,
  "paradise-biryani-hyderabad-yza567": 24,
  "vidyarthi-bhavan-bangalore-stu901": 31,
  "ashok-vada-pav-mumbai-mno345": 11,
  "lassiwala-jaipur-bcd890": 8,
  "kesar-da-dhaba-amritsar-hij456": 14,
  "peter-cat-kolkata-nop012": 19,
};

/**
 * Seed initial like/bookmark counts (runs once per device).
 */
export function initSocialData(): void {
  if (localStorage.getItem(SEEDED_KEY)) return;
  localStorage.setItem(LIKES_KEY, JSON.stringify(SEED_LIKES));
...
function getMyLikes(): string[] {
  try { return JSON.parse(localStorage.getItem(MY_LIKES_KEY) || "[]"); } catch { return []; }
}

function getBookmarkCounts(): Record<string, number> {
  try { return JSON.parse(localStorage.getItem(BOOKMARKS_KEY) || "{}"); } catch { return {}; }
}

function getMyBookmarks(): string[] {
  try { return JSON.parse(localStorage.getItem(MY_BOOKMARKS_KEY) || "[]"); } catch { return []; }
}

export function getLikeCount(placeId: string): number {
  return getLikeCounts()[placeId] || 0;
}

export function hasLiked(placeId: string): boolean {
  return getMyLikes().includes(placeId);
}

export function getMyLikedIds(): string[] {
  return getMyLikes();
}

export function getMyBookmarkedIds(): string[] {
  return getMyBookmarks();
}

export function toggleLike(placeId: string): { count: number; liked: boolean } {
  const liked = hasLiked(placeId);
  const counts = getLikeCounts();
  counts[placeId] = Math.max(0, (counts[placeId] || 0) + (liked ? -1 : 1));
  localStorage.setItem(LIKES_KEY, JSON.stringify(counts));

  let myLikes = getMyLikes();
  myLikes = liked ? myLikes.filter((id) => id !== placeId) : [...myLikes, placeId];
  localStorage.setItem(MY_LIKES_KEY, JSON.stringify(myLikes));

  return { count: counts[placeId], liked: !liked };
}

export function getBookmarkCount(placeId: string): number {
  return getBookmarkCounts()[placeId] || 0;
}

export function hasBookmarked(placeId: string): boolean {
  return getMyBookmarks().includes(placeId);
}

export function toggleBookmark(placeId: string): { count: number; bookmarked: boolean } {
  const bookmarked = hasBookmarked(placeId);
  const counts = getBookmarkCounts();
  counts[placeId] = Math.max(0, (counts[placeId] || 0) + (bookmarked ? -1 : 1));
  localStorage.setItem(BOOKMARKS_KEY, JSON.stringify(counts));

  let myBookmarks = getMyBookmarks();
  myBookmarks = bookmarked
    ? myBookmarks.filter((id) => id !== placeId)
    : [...myBookmarks, placeId];
  localStorage.setItem(MY_BOOKMARKS_KEY, JSON.stringify(myBookmarks));

  return { count: counts[placeId], bookmarked: !bookmarked };
}
